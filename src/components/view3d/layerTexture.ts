...
function serialiseElement(el: SvgElement, stroke: string, strokeWidth: number): string {
    ...
    switch (el.type) {
        case 'circle':
            return `<circle cx="${el.attrs.cx}" cy="${el.attrs.cy}" r="${el.attrs.r}" ${attrsToString(base)} />`;
        case 'line':
            return `<line x1="${el.attrs.x1}" y1="${el.attrs.y1}" x2="${el.attrs.x2}" y2="${el.attrs.y2}" ${attrsToString(base)} />`;
        case 'path':
            return `<path d="${el.attrs.d}" fill="${(el.attrs.fill as string) || 'none'}" ${attrsToString({ stroke: strokeAttr, 'stroke-width': swAttr })} />`;
        case 'rect':
            return `<rect x="${el.attrs.x}" y="${el.attrs.y}" width="${el.attrs.width}" height="${el.attrs.height}" ${attrsToString(base)} />`;
        default:
            return '';
    }
}
...
export function buildLayerSvgString(layer: Layer, canvas: CanvasSettings): string {
    ...
    const cx = width / 2;
    const cy = height / 2;
    const transform = `translate(${layer.offsetX}, ${layer.offsetY}) rotate(${layer.rotation}, ${cx}, ${cy}) scale(${layer.scale})`;
